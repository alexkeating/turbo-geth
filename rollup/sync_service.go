...
func NewSyncService(ctx context.Context, cfg Config, txpool *core.TxPool, db ethdb.RwKV, chainConfig *params.ChainConfig) (*SyncService, error) {
...
func GetBlockByNumber(db ethdb.RwKV, number uint64) *types.Block {
...
// caching it if found.
func GetBlock(db ethdb.RwKV, hash common.Hash, number uint64) *types.Block {
...
// Block chain utilities
// Write up utilities to replace the use of bc
func CurrentBlock(db ethdb.RwKV) *types.Block {
...
func (s *SyncService) initializeLatestL1(ctcDeployHeight *big.Int) error {
	...
	if index == nil {
		...
		block := GetBlockByNumber(s.db, *index-1)
		if block == nil {
			block = CurrentBlock(s.db)
